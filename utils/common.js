...
export function isApp() {
    ...
    // `);
    const isFullScreen =
        window.screen.height - document.body.offsetHeight <= 90;
...
export function getGameImg(game) {
    ...
    // PT
    if (game.gametype === 'PTGame') {
        path = `assets/third/pt/${encodeURIComponent(
            `${game.enname}`
        )}.jpg`;
    } else if (game.gametype === 'CQ9Game') { // CQ9
        path = `assets/third/cq9/${encodeURIComponent(
            `${game.gamecode}`
        )}.png`;
    } else {
        if (game.gamelsh) {
...
export function getGameTypeName(game) {
    const gameType = GAME_TYPES().find(
        (item) => item.value === game.biggametype
    );
