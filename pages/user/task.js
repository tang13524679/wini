...
export default function TaskPage() {
  ...
  function getStatus(item) {
    if (item.rewardtype == 1) {
      if (lang === "en") {
        return (
          <div className="line-clamp-1">
            <span>reward</span>
            <span className="clMainYellow mx-2">{item.rewardamount}</span>
      ...
      return (
        <div className="line-clamp-1">
          <span className="clMainYellow mr-2">{item.rewardamount}</span>
      ...
      <div className="task sm:px-4">
        <div className="bgInnerPage">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-4">
            {list.map((item, index) => (
              <div key={index} className="bgWhite6 p-3 rounded-md">
                ...
                <div className="flex justify-between items-center">
                  <Progress
                    percent={(item.completedAmount / item.taskvalue) * 100}
                    showInfo={false}
                    strokeColor="#10E070"
                  />
                </div>
