            ...
            <div
              ...
              onClick={async () => {
                requestApi(
                  ...
                  () => {
                    message.info(t("verifyCodeSent", "msg"));
                  }
                );
              }}
            >
...
const OtpMethodChooserModal = ({ current, open, onClose }) => {
  ...
  const options = methods.map(({ key, label, icon }) => (
    <button
      ...
      className={classNames(
        ...
        current === key ? "main-active-bg" : "bgWhite6 border border-white/5"
      )}
    >
      <span className={icon} />
      <span>{label}</span>
  ...
  return (
    <Modal open={open} closable={false} footer={null} centered={true}>
      <div className="p-4">
        <h3 className="clWhite text-base">{t("chooseOtpMethodTitle")}</h3>
