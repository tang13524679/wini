...
export default function VipUserProgress({
    ...
    const _currentLevel =
        typeof level === 'number' ? (
            <Image
                src={`/assets/vip-levels/vip_sz_0${level}_on.svg`}
                alt="icon"
                width="34"
                height="34"
            />
    ...
    const _nextLevel =
        typeof level === 'number' ? (
            level < 5 && (
                <Image
                    src={`/assets/vip-levels/vip_sz_0${level + 1}.svg`}
                    alt="icon"
                    width="34"
                    height="34"
                />
                ...
                <div className="mt-4 flex justify-between text-xs">
                    ...
                    <span className="space-x-1">
                        <span className="clWhite">
                            {formatMoney(currentAmount ?? 0)}
                        </span>
                        <span className="clWhite30">
                            / {formatMoney(nextAmount ?? 0)}
                        </span>
                    </span>
                </div>
            </div>
