      ...
      >
        <div className="p-4">
          <div className="flex justify-center items-center">
            ...
            ></div>
            <div className="clMainYellow">{receiveThirdParty.name}</div>
          </div>
          <div className="flex justify-center items-end mt-4">
            <div className="clWhite text-2xl">{formatMoney(amount)}</div>
            <div
              style={{ border: "1px solid #374220" }}
              className="clWhite30 ml-2"
            >
              VND
            </div>
          </div>

          <a href={payLink} target="_blank" className="block" rel="noreferrer">
            <div className="py-2 mt-5 rounded-full btnYellow">{t("toPay")}</div>
          </a>
        </div>
      </Modal>
      <div className="px-4">
        <div className="">
          ...
          >
              ...
              {thirdParties?.length > 0 &&
                thirdParties?.map((item, index) => (
                  <div
                    key={index}
                    className={`flex items-center bgWhite6 rounded-full h-12 pl-4 pr-2 cursor-pointer ${
                      receiveThirdParty.channelId === item.channelId
            ...
            <div className="clWhite mt-8">{t("amount", "field")}</div>
            <div
              style={{
              ...
              <div>
                ...
                {receiveBank.bankname && (
                  <div className="bgWhite2 rounded-md p-4">
                    <div className="my-1">{receiveBank.bankname}</div>
              ...
              onClick={() => {
                ...
                if (activeType === 0) {
                  ...
                  requestApi(
                    dispatch,
                    async () => {
                      const amountNum = convertMoney(amount);
                      if (!amountNum) throw t("required", "msg");
                      checker.isNumber(amountNum);
                      return fundApi.eSaving({
                        channelId: receiveThirdParty.channelId,
